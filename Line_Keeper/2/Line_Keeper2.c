...
void cal_lane_position(float * L_point,int line_no , float *lane_pos)
{
	...
	for(i=0;i<line_no-1;i++)
	{
		for(j=i;j<line_no;j++){
			...
			if((distance[i]>=LINE_WIDTH-LINE_WIDTH_TOL)&&(distance[i]<=LINE_WIDTH_TOL+LINE_WIDTH))
			printf("errer[%d %d] = %3.1lf\n" ,i,j ,distance[i]);
		}
...
int main(void){
	...
	mat_image_org_color = imread("/home/pi/HancomMDS/AutoCar/C++/Line_Keeper/2/images/line_2_0.jpg"); 
